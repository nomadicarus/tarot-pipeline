...
- [x] Refactor `prompts/builder.py` and `config/decks.json`: adopt web-interface prompt structure — negative prompts, rendering engine framing, master prompt template per deck, technical quality suffix. Inject per-card Thoth symbolism. Source: `thoth.json`, `claymation.json`, `notes-web2api.md`. → COMPLETE
